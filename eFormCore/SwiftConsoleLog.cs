...
namespace Microting.eForm
{
    public class SwiftConsoleLog : ISwiftLogger
    {
        ...
        {
            Console.WriteLine(string.Format(_authError, ex.InnerException != null ? ex.InnerException.Message : ex.Message, username, password, endpoint));
        }

        public void LogRequestError(Exception ex, HttpStatusCode statusCode, string reason, string requestUrl)
        {
            Console.WriteLine((string.Format(_requestError, reason, statusCode.ToString(), requestUrl)));
        }

        public void LogUnauthorizedError(string token, string endpoint)
        {
            Console.WriteLine((string.Format(_unauthorizedError, token)));
        }
    }
}